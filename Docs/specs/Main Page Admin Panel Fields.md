...
═══════════════════════════════════════════════════════════
РЕШЕНИЕ 9 (2026-09-14) — Кадрирование изображений перед загрузкой
═══════════════════════════════════════════════════════════
 • Новый компонент src/components/ImageCropperDialog.tsx: диалог на
   ui/Dialog; фиксированная рамка с пропорцией слота, панорамирование
   перетаскиванием (Pointer Events), зум слайдером / колесом / стрелками
   (1× fill … 4× fill); «Применить» рисует выбранную область в canvas —
   экспорт ≤1600px по большей стороне, PNG/WebP сохраняют тип (и
   прозрачность), остальное → JPEG 0.92.
 • ImageUploader: новый проп cropRatio — при выборе/drop файла (кроме SVG)
   открывается кадрирование, в R2 уезжает уже кадрированное изображение:
   превью в админке = публичный рендер (object-cover) без сюрпризов.
   Оригинал держится в памяти сессии — иконка Crop на превью пересоздаёт
   кадр; после перезагрузки страницы кнопка предлагает выбрать файл заново.
 • ImageUploaderField: прокидка cropRatio. Включено (пропорции = слотам):
   Avatar 1:1, Cover 16/5, About process image 516/495, OG 1200/630,
   Favicon 1:1.
 • Схема БД и /api/upload не менялись: files.width/height сервер считает
   уже по кадру; старые файлы остаются валидными (object-cover как раньше).
 • Уточнение UX (по фидбэку): модель «вписать и выделить» — при открытии
   видно ВСЁ фото, белая рамка = крупнейший вписанный прямоугольник
   пропорции слота, области за рамкой приглушены (box-shadow оверлей);
   зум 0 = «вписано», до 4×; панорамирование ограничено покрытием рамки.
   Дефолт «заполнить рамку» (одна голова для крупных фото) убран.
 • Фикс стабильности кроппера: размеры берутся из предзагрузки (new Image),
   картинка и рамка позиционируются явными px (без CSS-transform/scale),
   высота окна фиксированная 420px, сбои загрузки видны текстом в диалоге
   (раньше тихий сбой object URL оставлял пустое окно и «мёртвую» кнопку).
