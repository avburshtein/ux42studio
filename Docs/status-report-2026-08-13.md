...
**Да, добавлен.** Enum расширен до `['draft', 'published', 'archived']` (default `draft`) в `src/db/schema/projects.ts`. На уровне SQLite это не требует миграции DDL (колонка `status` — обычный `TEXT`, enum — только TypeScript-ограничение Drizzle), поэтому `drizzle-kit generate` корректно сообщает «No schema changes». Отдельно есть булев флаг `showOnHomepage` (глобальная видимость на главной, управляется суперадмином) и `publishedAt`. Механика перевода в `archived` и UI описаны в спеке `10-archived-status.md`.
...
### Роутинг публичных страниц

**Канонический вариант — префикс `/u/`.** Публичные маршруты:

- `GET /u/[slug]` — публичный профиль дизайнера + сетка его опубликованных проектов (`(public)/u/[slug]/page.tsx`).
- `GET /u/[slug]/[projectSlug]` — страница конкретного проекта (`(public)/u/[slug]/[projectSlug]/page.tsx`).

Ссылка «View» в суперадминке ведёт на `/u/${authorSlug}/${slug}`. Плоский путь `/[username]/[case-slug]` без `/u/` **не используется** и не планируется.
...
6. Роутинг зафиксирован: канонический вариант — `/u/[slug]/[projectSlug]` (префикс `/u/`).
7. Статус `archived` добавлен в схему (`['draft', 'published', 'archived']`). Страница `/admin/site` (тема/layout/сортировка/SEO) — вне текущего скопа, описана отдельным документом `Docs/specs/admin-site-plan.md`.
