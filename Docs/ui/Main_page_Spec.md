...
### Обновление 2026-09-16 — сортировка кейсов (v1)

**(60)** Порядок кейсов в публичной галерее — выбирает дизайнер в /admin
        (панель CaseSortManager над списком; один dropdown вместо пары
        Toggle+Dropdown — Manual это просто ещё один вариант):
        • Сначала новые (default, = прежнее поведение publishedAt DESC);
        • Сначала старые; A → Z; Z → A (localeCompare, case-insensitive;
          в SQL — COLLATE NOCASE);
        • Вручную (drag & drop): dnd-kit sortable, перетаскивание за
          ручку ⠿ (не конфликтует с кнопками строки), оптимистичный
          порядок + reorderProjects → projects.sort_order (asc).
        Хранение: profiles.case_sort_mode (NULL = 'newest') +
        projects.sort_order (backfill миграции — ранг по дате публикации,
        новейший = 0; новый кейс — max+1, в конец). Публичная страница
        (ISR 3600) инвалидируется revalidatePath в actions.
        Срезано из исходной задумки: «By Category» как сортировка →
        категорийный фильтр (уже есть); «иконка было→станет» в dropdown →
        не делаем (overengineering). By Cover Color — v2: доминантный
        цвет/hue считать клиентски в кроппере при экспорте обложки
        (на Workers нет Canvas), бэкфилл старых обложек утилитой;
        серые обложки (низкая насыщенность) — в конец по светлоте.

### Обновление 2026-09-18 — запас контраста тональных пар

**(61)** Пары из билдера M3 проходили AA впритык (4.55–4.56) — подтянуты
        только on-* тексты (визуально неотличимо), фоны не тронуты:
        • on-primary-container #98edc6 → #d9f9e7: 4.55 → 5.56 (активные
          пилюли навигации, чип «Опубликован», super-admin);
        • on-secondary-container #680010 → #44000a: 4.56 → 5.82
          (Button/Badge secondary);
        • light error-container #ba5f22/#fda98d → #ffdad6/#410002:
          2.37 FAIL → 13.26 — средний тон #ba5f22 не имел проходного
          текста в принципе (макс 4.42 с белым); чипы super-admin и
          кнопка удаления изображения теперь M3-корректные (светлый
          контейнер + тёмный текст). Тёмная пара (4.56) не менялась.
        Все остальные пары кнопок/бейджей — PASS в обеих темах
...
### Обновление 2026-09-18 — TL;DR-слой (короткая версия кейса)

**(63)** Короткая версия кейса для hiring manager / рекрутера —
        авто-генерируется из уже структурированных данных, дизайнер
        НЕ заполняет отдельных полей. Бонус к готовому кейсу.

        URL: /u/[slug]/[projectSlug]/short (отдельный path-сегмент, не
        query param — чище для шаринга, независимый ISR).

        Что входит (адаптивно — пустые поля не выводятся):
        • Hero: обложка + title + teaser + metadata-строка
          (client · year · duration · role) + category-теги;
        • Problem: первые 1–2 предложения problemStatement;
        • Goal: projectGoal (как есть);
        • Results: топ-3 keyMetrics (по order) — карточки с цифрами;
        • Key screens: 2–4 первых final_gallery assets (aspect 4/3);
        • Takeaway: keyTakeaway;
        • CTA «View full case study →» → полный кейс;
        • Footer: SiteFooter дизайнера (social links, Back to Gallery).

        Точки распространения (только в админке, не на витрине):
        Review-шаг → блок «Короткая версия (TL;DR)» с двумя кнопками:
        «Открыть TL;DR» (превью в новой вкладке) + «Скопировать короткую
        ссылку» (clipboard API + toast «Скопировано» + execCommand
        fallback). Кнопки видны только для опубликованных проектов.

        НЕ сделано (намеренно): переключатель на полном кейсе (не нужен
        — посетитель уже на полной версии), кнопки на карточках галереи
        (не показываем случайному посетелю альтернативу). Quick-view
        модалка — отдельная задача (3). PDF — отдельная задача (4).

### Обновление 2026-09-18 — фиксы TL;DR по фидбэку

**(64)** Правки страницы /short после просмотра:

        • Hero перестроен по паттерну полного кейса (Hero.tsx):
          обложка — карта в контейнере на ≥lg (rounded-b-3xl снизу),
          full-bleed на мобильных; заголовок + теги + teaser —
          absolute-оверлей ВНУТРИ обложки (пилюли тегов как на
          полном кейсе). Причина бага: текстовый блок с -mt шёл
          соседом после обложки и рисовался ПОД absolute-картинкой
          (Image fill) — stacking: positioned элементы всегда выше
          in-flow контента, title и теги «исчезали» под обложкой.
          Metadata-строка (client · year · duration · role) — под
          обложкой, по оси section-container.
        • Шапка — SiteHeaderBreadcrumb (Back + Portfolio / Cases /
          {title}), как на полном кейсе. Work/About на глубокой
          странице убраны: это якоря страницы профиля, на /short
          они не работают.

### Обновление 2026-09-18 — PDF через print-CSS (задача 4)

**(65)** Печатная версия TL;DR: hiring manager сохраняет /short как
        PDF штатным диалогом печати браузера. Ноль зависимостей,
        ноль генераторов — чистый @media print.

        • Кнопка «Скачать PDF» (PrintTldrButton, client) — плавающая
          справа внизу на странице /short, window.print(). Тултип
          «В диалоге печати выберите "Сохранить как PDF"» (2.5 с).
          print:hidden — в печатный лист не попадает.
        • ?print=1 — автозапуск диалога через 1.2 с (даём обложке и
          key screens догрузиться). useSearchParams — в Suspense-
          границе, чтобы не ронять статический рендер (ISR 3600).
        • Точка распространения: Review-шаг, блок TL;DR — третья
          кнопка «Скачать PDF» → открывает /short?print=1 в новой
          вкладке, диалог открывается сам.
        • Print-стили (globals.css, блок в конце файла, scope
          .print-short на корневом div страницы):
          – светлая палитра форсируется копией M3-light токенов в
            @media print (:root). Блок стоит после [data-theme='dark']
            — при равной специфичности побеждает позднее правило,
            печатаем всегда светлую тему, независимо от переключателя;
          – фон чисто белый (экономия чернил), обложка ужата до
            250px (cover-shell) — hero целиком на первой странице;
          – print-color-adjust: exact на cover-shell/скриме — белые
            пилюли тегов и градиент-затемнение сохраняются, заголовок
            поверх фото читаем на бумаге;
          – скрыты при печати: шапка (header), футер (footer),
            CTA-блок «View full case study» (.no-print), кнопка PDF;
          – карточки метрик и key screens — break-inside: avoid
            (.print-avoid), заголовок секции не отрывается от
...
### Обновление 67 (2026-09-18) — Reviews на витрине, мудборд, PDF, мелкие фиксы

**1. Reviews не выводились на странице кейса.** Причина: `projectReviews` вообще не
загружались и не рендерились в `page.tsx`. Добавлены: загрузка с `avatarFile` в
Promise.all + блок «Client reviews» в секции Reflection (цитата, аватар 40px,
имя, роль). Авто-скрытие, если отзывов нет.

**2. Мудборд: «плывущий» порядок/старые фото.** Причины: (а) `updateProjectGallery`
не сбрасывал кэш публичной страницы (`revalidate = 3600`) — добавлен
`revalidatePath('/', 'layout')`; (б) drag-n-drop в `AdminGridEditor` свапает
`slotIndex`, но не порядок массива, а сохранение ставило `order` по позиции в
массиве — сохранение теперь сортирует по `slotIndex`.

**3. Before/After (витрина): подписи Before/After — выравнивание по верху**
(`items-center` → `items-start`), текст описания при длинной строке не
сползает под label.

**4. PDF TL;DR: хедер/футер печатались, карточки рвались.** Корень: блок
`@media print` был случайно вложен в `@media (max-width: 767px)` и на десктопе
не применялся. Блок закрыт; дополнительно `break-inside: avoid` на figure/
button/blockquote — цельные блоки не разрываются между страницами, перенос
по гапам.

**5. Key Metrics: заголовок карточки** `headline-lg` → `headline-sm` — как в
TL;DR, помещается на мобильных (фидбэк).

**6. Hero mobile: заголовок/теги заезжали под шапку** — высота обложки
320→400px (sm 420→480), оверлею хватает места.

### Обновление 68 (2026-09-18) — обложка без срезания контента, Before/After

**Корень (витрина + TL;DR + PDF):** текстовый оверлей обложки был
`absolute bottom-0` внутри контейнера **фиксированной высоты** — длинный
заголовок/teaser/теги росли вверх и срезались верхним краем (обрезанный тег).

**1. Hero (полный кейс) и CaseShortView (/short):** оверлей переведён в
in-flow — контейнер `flex min-h-[…] flex-col justify-end`, контент
`relative z-10` с верхним паддингом; картинка (Image fill) и скрим —
absolute-подложка. Контейнер теперь **растёт по контенту**, ничего не
срезается ни на одном экране; gradient-fallback тоже `absolute inset-0`.

**2. PDF (.cover-shell в @media print):** `height: 250px !important` →
`height: auto !important; min-height: 250px !important` — обложка в PDF
растягивается под заголовок/теги/teaser (была та же проблема, что на
мобильной витрине).

**3. Before/After (витрина):** лейблы BEFORE/AFTER перенесены НАД
изображением, текст описания — под картинкой на всю ширину карточки
(раньше inline-лейбл рядом с текстом формировал отступ и текст не
выравнивался по краю карточки).
