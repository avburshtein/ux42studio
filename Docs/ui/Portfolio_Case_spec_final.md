...
Раунд 6 (критический фикс сборки, 2026-09-01):

(17) «Сломалась ширина контента»: продакшен-сборка (next build
    --webpack) падала — «globals.css:2:67438 Missed semicolon». Причина:
    сканер Tailwind v4 читает ВСЕ файлы вне .gitignore, включая
    справочные Docs/make-export/** (Figma Make export, коммит
    6878934), и генерирует из кандидата font-['Inter:Medium'] невалидное
    правило font-family: \\'Inter:Medium\\' — postcss не может
    распарсить вывод. Бисектир показал: сломано с 6878934, т.е. все
    последние деплои не могли пройти, а в dev (Turbopack) браузер
    терял часть CSS после битого правила — «всё растягивается».
    Фикс: @source not "../../Docs" в globals.css — справочники
    исключены из сканирования. Бисектир: сломано на 5c503d0/6158931/
    cce675a/HEAD до фикса, PARSE_OK после; next build --webpack —
    успешен. После фикса обязателен рестарт dev-сервера и деплой.
