...
export const projects = sqliteTable('projects', {
  ...
  // Statuses & Visibility
  status: text('status', { enum: ['draft', 'published'] }).notNull().default('draft'), //[cite: 1]
  ...
  unqProfileSlug: uniqueIndex('idx_projects_profile_slug').on(t.profileId, t.slug),
  homeIdx: index('idx_projects_home').on(t.status, t.showOnHomepage, t.createdAt),
}));
...
export const projectAssets = sqliteTable('project_assets', {
  ...
  assetType: text('asset_type', { enum: ['moodboard', 'wireframe', 'final_gallery'] }).notNull(), //[cite: 1]
